...
param createApplicationInsights = false
```

Set `createApplicationInsights` to `false` (or set the `CREATE_APPLICATION_INSIGHTS` environment variable to `false`) to skip the APIM, Function/Logic App, and Foundry Application Insights resources and their dependent integrations. Log Analytics diagnostics remain enabled.
