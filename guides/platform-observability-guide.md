...
**Optional Application Insights resources.** Set `createApplicationInsights = false`, or set `CREATE_APPLICATION_INSIGHTS=false` in the azd environment, to skip all three Application Insights components and their APIM, Function/Logic App, and Foundry integrations. APIM diagnostic settings continue to send logs directly to the Log Analytics workspace.
