...
@description('Name of the Azure Monitor Private Link Scope. Leave blank to use default naming conventions.')
...
@description('Name of the App Service Plan hosting the usage processing Logic App. Leave blank to use default naming conventions.')
...
@description('Use existing Private DNS Zones instead of creating and linking new zones. This mode is also enabled automatically when any resource ID is supplied in existingPrivateDnsZones.')
param useExistingDnsZones bool = false

@description('Create virtual network links in the Private DNS Zones supplied through existingPrivateDnsZones. Requires permissions on each DNS zone resource group.')
param createExistingDnsZoneVnetLinks bool = false
...
@description('Logic App inbound public network access.')
...
@description('Create Application Insights resources for APIM, Function/Logic App, and Foundry.')
...
// A single supplied zone ID switches the entire deployment to existing-zone mode.
// The explicit toggle also supports the legacy resource group/subscription lookup.
var hasExistingPrivateDnsZoneIds = !empty(existingKeyVaultDnsZoneId) || !empty(existingMonitorDnsZoneId) || !empty(existingEventHubDnsZoneId) || !empty(existingCosmosDbDnsZoneId) || !empty(existingStorageBlobDnsZoneId) || !empty(existingStorageFileDnsZoneId) || !empty(existingStorageTableDnsZoneId) || !empty(existingStorageQueueDnsZoneId) || !empty(existingCognitiveServicesDnsZoneId) || !empty(existingApimGatewayDnsZoneId) || !empty(existingAiServicesDnsZoneId) || !empty(existingOpenAiDnsZoneId) || !empty(existingRedisDnsZoneId)
var useExistingDnsZonesResolved = useExistingDnsZones || hasExistingPrivateDnsZoneIds
var existingPrivateDnsZoneResourceIds = union(
  !empty(existingKeyVaultDnsZoneId) ? [existingKeyVaultDnsZoneId] : [],
  !empty(existingMonitorDnsZoneId) ? [existingMonitorDnsZoneId] : [],
  !empty(existingEventHubDnsZoneId) ? [existingEventHubDnsZoneId] : [],
  !empty(existingCosmosDbDnsZoneId) ? [existingCosmosDbDnsZoneId] : [],
  !empty(existingStorageBlobDnsZoneId) ? [existingStorageBlobDnsZoneId] : [],
  !empty(existingStorageFileDnsZoneId) ? [existingStorageFileDnsZoneId] : [],
  !empty(existingStorageTableDnsZoneId) ? [existingStorageTableDnsZoneId] : [],
  !empty(existingStorageQueueDnsZoneId) ? [existingStorageQueueDnsZoneId] : [],
  !empty(existingCognitiveServicesDnsZoneId) ? [existingCognitiveServicesDnsZoneId] : [],
  !empty(existingApimGatewayDnsZoneId) ? [existingApimGatewayDnsZoneId] : [],
  !empty(existingAiServicesDnsZoneId) ? [existingAiServicesDnsZoneId] : [],
  !empty(existingOpenAiDnsZoneId) ? [existingOpenAiDnsZoneId] : [],
  !empty(existingRedisDnsZoneId) ? [existingRedisDnsZoneId] : []
)
...
module dnsDeployment './modules/networking/dns.bicep' = [for privateDnsZoneName in privateDnsZoneNames: if(!useExistingDnsZonesResolved) {
...
module vnet './modules/networking/vnet.bicep' = if(!useExistingVnet) {
  ...
  params: {
    ...
    location: location
    tags: tags
    privateDnsZoneNames: useExistingDnsZonesResolved ? [] : privateDnsZoneNames
...
module existingDnsZoneVnetLink './modules/networking/existing-dns-zone-vnet-link.bicep' = [for dnsZoneResourceId in existingPrivateDnsZoneResourceIds: if (createExistingDnsZoneVnetLinks && useExistingDnsZonesResolved) {
  name: 'existing-dns-vnet-link-${uniqueString(dnsZoneResourceId, vnetName)}'
  scope: resourceGroup(split(dnsZoneResourceId, '/')[2], split(dnsZoneResourceId, '/')[4])
  params: {
    dnsZoneName: split(dnsZoneResourceId, '/')[8]
    virtualNetworkId: useExistingVnet ? vnetExisting.outputs.virtualNetworkId : vnet.outputs.virtualNetworkId
...
module monitoring './modules/monitor/monitoring.bicep' = {
  ...
  params: {
    ...
    privateLinkScopeName: !empty(azureMonitorPrivateLinkScopeName) ? azureMonitorPrivateLinkScopeName : 'ampls-nprd-hub-uks-001'
    ...
    dnsZoneRG: !useExistingDnsZonesResolved ? resourceGroup().name : dnsZoneRG
...
module foundry 'modules/foundry/foundry.bicep' = {
  ...
  params: {
    ...
    vNetRG: useExistingVnet ? vnetExisting.outputs.vnetRG : vnet.outputs.vnetRG
    privateEndpointSubnetName: useExistingVnet ? vnetExisting.outputs.privateEndpointSubnetName : vnet.outputs.privateEndpointSubnetName
    aiFoundryPrivateEndpointBaseName: aiFoundryPrivateEndpointName
...
module apim './modules/apim/apim.bicep' = {
  ...
  params: {
    ...
    enableApplicationInsights: createApplicationInsights
