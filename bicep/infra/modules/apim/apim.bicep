...
resource applicationInsights 'Microsoft.Insights/components@2020-02-02' existing = if (enableApplicationInsights) {
...
resource appInsightsConnectionStringSecret 'Microsoft.KeyVault/vaults/secrets@2023-07-01' = if (enableApplicationInsights) {
...
resource appInsightsLoggerCredentialsNamedValue 'Microsoft.ApiManagement/service/namedValues@2024-06-01-preview' = if (enableApplicationInsights) {
...
resource apimLogger 'Microsoft.ApiManagement/service/loggers@2024-05-01' = if (enableApplicationInsights) {
...
resource apimAppInsights 'Microsoft.ApiManagement/service/diagnostics@2024-05-01' = if (enableApplicationInsights) {
  parent: apimService
...
resource ehUsageLogger 'Microsoft.ApiManagement/service/loggers@2022-08-01' = {
  ...
  properties: {
    ...
    credentials: {
      name: eventHubName
      endpointAddress: replace(replace(replace(eventHubEndpoint, 'https://', ''), ':443', ''), '/', '')
      identityClientId: managedIdentity.properties.clientId
    }
  }
}
...
resource ehPIIUsageLogger 'Microsoft.ApiManagement/service/loggers@2022-08-01' = if (enablePIIAnonymization) {
  ...
  properties: {
    ...
    credentials: {
      ...
      endpointAddress: replace(replace(replace(eventHubPIIEndpoint, 'https://', ''), ':443', ''), '/', '')
      identityClientId: managedIdentity.properties.clientId
    }
  }
}
...
module weatherMCP './mcp-from-api.bicep' = if (isMCPSampleDeployed && enableApplicationInsights) {
  name: 'weather-mcp'
  params: {
    apimServiceName: apimService.name
...
module microsoftLearnMCPServer 'mcp-existing.bicep' = if (isMCPSampleDeployed && enableApplicationInsights) {
